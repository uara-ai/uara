...
interface WhoopCardsProps {
  ...
  initialStats?: WhoopStats | null;
}

export function WhoopCards({ initialData, initialStats }: WhoopCardsProps) {
  const [whoopData, setWhoopData] = useState<WhoopDataResponse | null>(
    initialData || null
  );
  const [whoopStats, setWhoopStats] = useState<WhoopStats | null>(
    initialStats || null
  );
  ...
  const { execute: fetchWhoopData, isPending } = useAction(getWhoopDataAction, {
    onSuccess: async (result) => {
      if (result.data) {
        ...
        const stats = await processWhoopDataToStats(result.data);
        setWhoopStats(stats);
      }
    },
    onError: (error) => {
  ...
  const handleRefresh = () => {
    fetchWhoopData({ days: 30, limit: 100 });
  };
