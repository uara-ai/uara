...
export function WhoopStrainChart({ isOpen, onClose }: WhoopStrainChartProps) {
  ...
  const title = strainData?.data?.title || "Strain Analysis";
  const stage = strainData?.data?.stage || "loading";
  const progress = strainData?.data?.progress || 0;
  const chartData = strainData?.data?.chartData || [];
  const summary = strainData?.data?.summary;
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#085983]/10">
          <div className="flex items-center space-x-3">
            <div className="p-2 rounded-lg bg-[#085983]/10">
              <IconActivity className="h-5 w-5 text-[#dc2626]" />
            </div>
            <div>
              <h2 className="text-xl font-[family-name:var(--font-instrument-serif)] font-medium text-[#085983]">
                {title}
              </h2>
              {stage === "complete" && summary && "strainTrend" in summary && (
                ...
                {chartData.length === 0 ? (
                  <div className="bg-white rounded-xl p-8 border border-[#085983]/10 text-center">
                    <div className="p-4 rounded-lg bg-[#085983]/10 mx-auto w-fit mb-4">
                      <IconActivity className="h-8 w-8 text-[#dc2626]" />
                    </div>
                    ...
                    <p className="text-sm text-[#085983]/60 font-[family-name:var(--font-geist-sans)]">
                      {stage === "complete"
                        ? "The analysis completed but no strain data was found."
                        : "Strain data will appear once the analysis is complete."}
                    </p>
                  </div>
                ) : (
                  <>
